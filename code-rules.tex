...
\begin{flushleft}
{\Large
\textbf\newline{10 simple rules for making your software outlive your job}
}
\newline
\\
...
This article presents ten rules to help researchers ensure that
...
In some countries, the situation has recently worsened dramatically due to changes in the political and economic climate,
and in others such instability is long-standing.
In such situations, researchers in all disciplines may face the loss of funding or jobs,
...
Ideally, research software projects should be \emph{resilient} to such societal instability.
Historically,
...
Unfortunately, this may mean that the challenges of responding to macro-level changes in software development support are new and unfamiliar to research software developers,
and they may not have put structures in place to ensure that their code will outlast situations such as changing workplaces or careers,
partnerships failing,
a lab closing down,
or tooling and digital infrastructure becoming unavailable.
We therefore present ten rules to ensure that your software remains usable by others
even if you are no longer able to work on it.
Some of these rules are most useful when adopted at the beginning of a project
to build resilience over time (e.g., Rules 1, 2, 5, and 7),
...
or to \emph{sunsetting} it (i.e., phasing out development and support gracefully).
Even if your job is not in danger,
these rules will help others access and use your code,
...
Our first actionable rule is therefore to find out what those policies are,
e.g.,
...
\begin{quote}
  ...
  you may want to be selective about which steps you take proactively
...
\section*{Rule 2: Define your threat model}
...
The Open Worldwide Application Security Project (OWASP) provides definitions, guidance, and resources for software threat modeling through the Threat Modeling Project website.\footnote{\url{https://owasp.org/www-project-threat-modeling/}}
As with all crisis planning,
...
\item
  ...
  As the public face of a project, they may be more susceptible than other project members to threats such as doxxing or being personally targeted in the media because of their work.
  Rule~3 discusses ways to plan for this.
...
\item
  ...
  Regional and national governments plan for disasters like these
  by having evacuation plans to get victims to safe(r) places
  and provide them with emergency shelter and supplies.
...
When performing a threat analysis,
it is okay to decide that \emph{for your project} there are certain kinds of threats that you do not have a plan for (or the plan is ``abandon the project until further notice'').
Making those decisions before the threat arises means one less thing to cogitate on during the crisis,
in the unfortunate case where threat does materialize.

\section*{Rule 3: Decide if you are ending, pausing, or handing off}
...
as discussed in the rules that follow.
...
If you're not currently facing an abrupt transition in project leadership, developing a \emph{software management plan} now can help clarify the roles and responsibilities of running a project,
making it easier to reason about filling those roles when a decision about succession, suspension, or abandonment becomes necessary. The Software Sustainability Institute has resources\footnote{\url{https://www.software.ac.uk/guide/writing-and-using-software-management-plan}} to guide the development of software management plans.
...
A fiscal sponsor gives the software a neutral home
where it is no longer owned by a single research organization,
which can help to insulate it from changes in policy.
Fiscal sponsors also tend to have lower overhead and procurement costs compared to
university or government entities.\footnote{That said, if your institution has an Open-Source Program Office (OSPO),
that can be a source of stability, for example through small internal grants or through
connections to a network of possible future co-maintainers or successors for your project.}
...
\section*{Rule 4: Choose an open license (if you can)}
...
or may simply be unsure whether they need permission or not.
...
Note also that if you hope to someday commercialize the research software you have written, consider carefully whether an open-source license will interfere with that eventuality.
...
\begin{quote}
  ...
  in the same way as discussion of fiscal sponsorship (Rule~3).
  For example,
...
\section*{Rule 5: Save everything in multiple places}

Now that you are legally able to share your code, remember LOCKSS: Lots of Copies Keep Stuff Safe.\footnote{A related mnemonic is ``3-2-1'': 3 copies, on 2 different media, 1 in a different location.}
But copies are not enough:
the threats to your project may be political as well as technological (Rule~2),
...
Remote Git repositories can also be self- or institution-hosted;
these typically lack the social and interactive features of the commercial options,
but (depending on the institution or private host's location/jurisdiction) may provide some protection against the aforementioned correlated threats.
...
\section*{Rule 6: Encourage community adoption}
...
It can also help to familiarize yourself with \emph{other} tutorial resources that your
(future) users or contributors might need.
For example, point users to resources such as Software Carpentry\footnote{\url{https://software-carpentry.org/}} to learn the basics of
command-line shells \emph{prior to your tutorial or workshop} so that you can spend
more of that time teaching them about your software.

Deeper, mentored engagements with your software like sprints, class projects,
or Google Summer of Code\footnote{\url{https://summerofcode.withgoogle.com/}}
can go even further toward preparing someone else to co-maintain your software
or take the reins when you step down.
The obvious trade-off is that (compared to a workshop)
mentorship is a larger investment of time in a smaller number of people.
If you have students or post-docs under your direction,
mentoring them in research software development and maintenance
alongside your scientific mentorship can also increase your project's chances of longevity.
...
\section*{Rule 7: Do what you're supposed to (if you have time)}
...
\cite{Sandve2013,Wilson2014},
this rule is a prioritized checklist of things to do
\emph{if you are not facing immanent involuntary changes to your employment}.
If you are,
please consult the next rule.
...
\item
  ...
  Make the data public (if allowed) and easy to access by your users, either by automating its download or
  incorporating the data in your project if possible.
...
The above suggestions will help to reduce your project's \emph{technical~debt}, but are not a panacea.
Using descriptive variable names, adding in-line documentation,
refactoring near-duplicated code into reusable functions, adopting automated code linting,
setting up continuous integration testing,
and many other widely-touted best practices will help make your project more resilient
by making the software less effortful to maintain.
...
(as discussed in our next rule).

\section*{Rule 8: Do what you can (if you have to act quickly)}
...
\section*{Rule 9: Talk about what you're doing}
...
Share your own rules on social media
...
Having years of work come to an end can take a toll on your mental health,
even if it stems from a \emph{voluntary} career transition.
If your transition is triggered by institutional or societal factors, many of your colleagues may be suffering as well,
...
(and/or labor unions, if your job class is affiliated with one)
and push them to take meaningful action.
Professional societies are often better placed than individuals to exert pressure on government officials,
and revising a society's bylaws and budget to prioritize safety-net programs (such as bridge funding) for its members can be pursued as well.
...
It is our hope that by documenting some recommendations based on the collective experience and wisdom of researchers across the globe,
we may empower others to make their own software projects more resilient.
However, we left many suggestions out of this paper due to space constraints
or because we felt they would age poorly.
Other ideas may be omitted because
